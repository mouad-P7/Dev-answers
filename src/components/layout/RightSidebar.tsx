...
export default async function RightSidebar() {
  ...
  return (
    <aside className="background-light900_dark200 light-border custom-scrollbar text-dark500_light700 sticky right-0 top-0 flex h-screen w-[280px] flex-col gap-6 overflow-scroll border-l p-3 pt-24 shadow-light-300 dark:shadow-none max-xl:hidden">
      <section>
        ...
        <div className="flex-between flex-col gap-4">
          {hotQuestions.map((qst) => (
            ...
            >
              <p className="body-medium line-clamp-2">{qst.title}</p>
              <Image
        ...
        <div className="flex-between flex-col gap-4">
          {popularTags.map((tag) => (
            <div key={tag._id} className="flex-between w-full">
              ...
              <p className="small-medium flex-between gap-2">
                {formatNumber(tag.questionsCount)}
                <span>
                  {tag.questionsCount <= 1 ? "Question" : "Questions"}
                </span>
              </p>
            </div>
          ))}
